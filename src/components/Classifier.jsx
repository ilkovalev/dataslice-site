  ...
  return (
    <div className="rounded-xl border border-black/10 bg-panel p-5">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto select-none">
        ...
        <text x={W - PAD} y={16} fill="#2ab8eb" fontSize="10" textAnchor="end">→ помечаем спамом</text>
        <text x={8} y={SPAM_Y + 4} fill="#2ab8eb" fontSize="11">спам</text>
        <text x={8} y={HAM_Y + 4} fill="#6b7280" fontSize="11">не спам</text>
        ...
        <text x={sx(thr)} y={H - 6} fill="#2a2f3a" fontSize="10" textAnchor="middle">порог</text>
      </svg>

      <div className="grid sm:grid-cols-2 gap-4 mt-3">
        <div className="text-xs text-gray-600">
          <div className="grid grid-cols-3 gap-px text-center">
            <span></span><span className="text-gray-500">пред. спам</span><span className="text-gray-500">пред. не спам</span>
            <span className="text-gray-500 self-center text-left">реально спам</span><span className="bg-[#2ab8eb]/15 py-1 rounded">TP {tp}</span><span className="bg-[#f87171]/15 py-1 rounded">FN {fn}</span>
            <span className="text-gray-500 self-center text-left">реально не спам</span><span className="bg-[#fbbf24]/15 py-1 rounded">FP {fp}</span><span className="bg-black/5 py-1 rounded">TN {tn}</span>
          </div>
        </div>
        <div className="text-sm space-y-1">
          {stat('Precision (точность)', precision, 'text-cyanink')}
          {stat('Recall = TPR (полнота)', recall, 'text-cyanink')}
          {stat('FPR (ложные срабатывания)', fpr, 'text-amber-600')}
          {stat('Accuracy (доля верных)', accuracy, 'text-gray-700')}
        </div>
      </div>
      ...
      <p className="text-xs text-gray-500 mt-2">Двигайте порог — Precision и Recall обмениваются. Хуже разделимость (классы перекрыты) — обе метрики падают. Accuracy обманчива на дисбалансе: если спама мало, «всё не спам» даёт высокую accuracy, но Recall = 0.</p>
    </div>
  )
}
