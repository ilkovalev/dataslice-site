  ...
  return (
    <div className="rounded-xl border border-black/10 bg-panel p-5">
      ...
      <div className="flex flex-wrap gap-4 mt-1 text-sm">
        ...
        <span className={sig ? 'text-green-600' : 'text-gray-600'}>p-value = {pval < 0.001 ? '<0.001' : pval.toFixed(3)} {sig ? '→ значимо' : '→ не значимо'}</span>
        <span className="text-gray-500">разница {((p2 - p1) * 100).toFixed(1)} п.п.</span>
      </div>

      <div className="grid sm:grid-cols-3 gap-3 mt-4 text-sm">
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Конверсия A</span><span className="text-cyanink">{(p1 * 100).toFixed(0)}%</span></div>
        ...
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Конверсия B</span><span className="text-cyanink">{(p2 * 100).toFixed(0)}%</span></div>
      ...
      <p className="text-xs text-gray-500 mt-2">Условия: большие n, метрика-доля (конверсия). Та же разница долей при большом n даёт большой |z| и маленький p — поэтому размер выборки решает.</p>
    </div>
  )
}
