...
const WORDS = [
  { w: 'выигрыш', s: 0.30, h: 0.02 },
  { w: 'срочно', s: 0.25, h: 0.05 },
  { w: 'деньги', s: 0.22, h: 0.05 },
  { w: 'встреча', s: 0.05, h: 0.30 },
  { w: 'отчёт', s: 0.04, h: 0.28 },
  { w: 'проект', s: 0.05, h: 0.22 },
]
...
export default function NaiveBayes() {
  ...
  return (
    <div className="rounded-xl border border-black/10 bg-panel p-5">
      <div className="text-xs text-gray-500 mb-2">Письмо = выбранные слова. Кликайте, чтобы включить/убрать слово:</div>
      ...
      <div className="mt-4 rounded-lg border border-black/10 bg-ink px-3 py-2.5 text-sm">
        <div className="font-mono text-gray-700 text-xs leading-relaxed">
          шансы = {prior.toFixed(2)}/{(1 - prior).toFixed(2)}
          {present.map((pr) => <span key={pr.w}> × {pr.lr.toFixed(1)}<span className="text-gray-400">({pr.w})</span></span>)}
        </div>
        <div className="mt-2 text-lg">P(спам | письмо) = <span className={`font-semibold ${p > 0.5 ? 'text-[#dc4d4d]' : 'text-green-600'}`}>{(p * 100).toFixed(1)}%</span> <span className="text-sm text-gray-500">→ {p > 0.5 ? 'спам' : 'не спам'}</span></div>
        {/* шкала */}
      ...
      <label className="block mt-3 text-sm">
        <div className="flex justify-between text-gray-700 mb-1"><span>Априорная доля спама P(спам)</span><span className="tabular-nums text-cyanink">{(prior * 100).toFixed(0)}%</span></div>
      ...
      <p className="text-xs text-gray-500 mt-2">Каждое спам-слово (LR&gt;1) толкает вероятность вверх, «деловое» (LR&lt;1) — вниз. Слова перемножаются независимо — отсюда «наивный». Базовая ставка P(спам) — стартовые шансы: при редком спаме даже сильные слова не делают письмо спамом наверняка.</p>
    </div>
  )
}
