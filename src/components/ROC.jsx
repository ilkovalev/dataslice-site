...
export default function ROC({ locale = 'ru' }) {
  const en = locale === 'en'
  ...
  return (
    <div className="rounded-xl border border-black/10 bg-panel p-5">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-sm mx-auto h-auto select-none">
        ...
        <text x={W / 2} y={H - 10} fill="#6b7280" fontSize="11" textAnchor="middle">{en ? 'FPR (false positives) →' : 'FPR (доля ложноположительных) →'}</text>
      ...
      <div className="flex flex-wrap gap-4 mt-1 text-sm">
        ...
        <span className="text-[#fbbf24]">{en ? 'current threshold:' : 'текущий порог:'} TPR {(cur.tpr * 100).toFixed(0)}% / FPR {(cur.fpr * 100).toFixed(0)}%</span>
      </div>
