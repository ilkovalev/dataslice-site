...
const ACTIONS = {
  ...
  en: {
    keep: { label: 'keep', note: 'The real outlier stays. It inflates the mean — for a "typical" value use the median.' },
    cap: { label: 'cap at p95', note: 'The extreme is clipped to p95: the observation survives, its influence is limited.' },
...
const SVG_L = {
  ru: { mean: 'среднее', median: 'медиана', outlier: (v, removed) => `выброс ${v} →${removed ? ' удалён' : ' обработан'}`,
    note: (m) => `Сравните: с выбросом среднее ≈ ${m}, медиана устойчива. Среднее скачет от одного значения, медиана — почти нет. Сначала поймите природу выброса (ошибка или реальность), потом выбирайте действие — и документируйте его.` },
  en: { mean: 'mean', median: 'median', outlier: (v, removed) => `outlier ${v} →${removed ? ' removed' : ' handled'}`,
    note: (m) => `Compare: with the outlier the mean is ≈ ${m}, the median barely moves. One value swings the mean; the median holds. First understand the outlier's nature (error or reality), then pick the action — and document it.` },
}

export default function OutlierActions({ locale = 'ru' }) {
  const A = ACTIONS[locale] ?? ACTIONS.ru
  const l = SVG_L[locale] ?? SVG_L.ru
  ...
  return (
    <div className="rounded-xl border border-black/10 bg-panel p-5">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto select-none">
        ...
        {full.filter((v) => v > 100 && act !== 'keep').map((v, i) => <text key={i} x={W - PAD} y={64} fill="#9ca3af" fontSize="9" textAnchor="end">{l.outlier(v, act === 'remove')}</text>)}
        <line x1={sx(mean)} y1={40} x2={sx(mean)} y2={86} stroke="#16a34a" strokeWidth="2" />
        <text x={sx(mean)} y={34} fill="#16a34a" fontSize="10" textAnchor="middle">{l.mean} {mean.toFixed(0)}</text>
        <line x1={sx(median)} y1={54} x2={sx(median)} y2={100} stroke="#fbbf24" strokeWidth="2" />
        <text x={sx(median)} y={112} fill="#d9a300" fontSize="10" textAnchor="middle">{l.median} {median}</text>
      </svg>
