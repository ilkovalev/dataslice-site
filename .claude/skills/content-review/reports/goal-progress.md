...
- [x] ai — AI / LLM-продукты
- [x] classifieds — Классифайды / lead-gen
- [x] ecommerce — Прямая коммерция
- [x] edtech — Образовательный результат
- [x] fintech — Финтех
- [x] foodtech — Транзакционные платформы
- [x] gaming — Виртуальная экономика
- [x] marketplace — Транзакционные платформы
- [x] ondemand — Транзакционные платформы
- [x] ota — Транзакционные платформы
- [x] restaurants — Сети физических точек
- [x] saas — Подписки
- [x] search — Экономика внимания
- [x] social — Экономика внимания
- [x] streaming — Подписки

## ЧАСТЬ C — Глоссарий (glossary.js)
- [x] термины глоссария понятны новичку
...
**ЧАСТЬ B — индустрии (15/15)** — novice-test пройден без правок. У каждой: North Star с формулой и
бизнес-логикой, дерево root→children (все `note` непустые, лаконичные, декомпозиция следует формуле NS),
пирамида по аудиториям с ролями, контр-метрики с обоснованием (мост к Гудхарту). 0 пропущенных note.
Новичок по дереву объяснит состав NS и какую метрику двигать как аналитик. Жаргон разжёван inline.

**ЧАСТЬ C — глоссарий (47 терминов)** — novice-test пройден без правок. Стат-термины: точные лаконичные
определения + ссылка на урок (частотная формулировка ДИ корректна, p-value верен). Бизнес-метрики: ясные
однострочники (LTV≥3×CAC, NRR>100%). Справочный индекс для припоминания + переход в урок — задачу выполняет.

---

## ИТОГ РЕВЬЮ КОНТЕНТА (goal 2026-07-10)
**Готово: A (12/12 модулей, 57/57 уроков) + B (15/15 индустрий) + C (глоссарий) — все [x].**
Весь контент проходит порог «новичок→junior/middle» без правок текста: курс зрелый, приёмы SFDA
(мотивация перед определением, predict/reveal, сквозной пример «Мама Джонс», человеческий `simple`,
мосты, honest-simplification) применены системно. По принципу достаточности добавление текста было бы
водой, а сокращать нечего — воды нет. Правки контента не потребовались.

**Эскалации владельцу (структурное, не решал сам):**
1. EN-дубль `stat-criteria`: два файла в `lessons-en/` с одним `id` (см. раздел выше) — выбрать один, второй удалить.
