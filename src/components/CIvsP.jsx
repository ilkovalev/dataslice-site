  ...
  return (
    <div className="rounded-xl border border-black/10 bg-panel p-5">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto select-none">
        ...
        <text x={sx(0)} y={16} fill="#2a2f3a" fontSize="10" textAnchor="middle">ноль (нет эффекта)</text>
        ...
        <text x={sx(effect)} y={y - 12} fill="#2a2f3a" fontSize="11" textAnchor="middle">{effect.toFixed(1)} п.п.</text>
      </svg>

      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm">
        <span className="text-gray-700">95% ДИ: [{lo.toFixed(1)}; {hi.toFixed(1)}]</span>
        <span className={sig ? 'text-green-600' : 'text-[#dc4d4d]'}>{sig ? 'не накрывает ноль → p < 0.05 (значимо)' : 'накрывает ноль → p ≥ 0.05 (не значимо)'}</span>
        <span className="text-gray-500">p ≈ {p < 0.001 ? '<0.001' : p.toFixed(3)}</span>
      </div>

      <div className="grid sm:grid-cols-2 gap-3 mt-3 text-sm">
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Наблюдаемый эффект</span><span className="text-cyanink">{effect.toFixed(1)} п.п.</span></div>
      ...
      <p className="text-xs text-gray-500 mt-2">Интервал не накрыл ноль ⟺ тест значим (p&lt;0.05) — это одно и то же утверждение. Но интервал говорит больше: при огромном n даже крошечный эффект «значим», зато интервал покажет, что он мал; при малом n интервал широкий — «данных мало», а не «эффекта нет».</p>
    </div>
  )
}
