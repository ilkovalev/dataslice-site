...
export default function InteractionPlot() {
  ...
  const verdict = interMag < 0.015 ? 'линии почти параллельны → взаимодействия нет' : interMag < 0.06 ? 'линии расходятся → слабое взаимодействие' : 'линии сильно расходятся/пересекаются → сильное взаимодействие'
  ...
  return (
    <div className="rounded-xl border border-black/10 bg-panel p-5">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-md mx-auto h-auto select-none">
        ...
        <text x={sx(0)} y={H - PAD + 16} fill="#6b7280" fontSize="11" textAnchor="middle">контроль</text>
        <text x={sx(1)} y={H - PAD + 16} fill="#6b7280" fontSize="11" textAnchor="middle">тест (B)</text>
        <text x={PAD} y={PAD - 8} fill="#9a907c" fontSize="10" textAnchor="start">конверсия</text>
      </svg>
      ...
      <div className="grid sm:grid-cols-2 gap-3 mt-3 text-sm">
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Главный эффект A (тест vs контроль)</span><span className="text-cyanink">{(effA * 100).toFixed(0)} п.п.</span></div>
        ...
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Взаимодействие A×B</span><span className="text-cyanink">{(inter * 100).toFixed(0)} п.п.</span></div>
      ...
      <p className="text-xs text-gray-500 mt-2">Поставьте взаимодействие в 0 — линии станут параллельны: эффект теста одинаков на mobile и desktop. Увеличьте — линии расходятся и могут пересечься: на mobile тест помогает, на desktop вредит. Значимое взаимодействие = «средний эффект» врёт, нужно смотреть по сегментам.</p>
    </div>
  )
}
