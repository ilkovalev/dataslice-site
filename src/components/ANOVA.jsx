  ...
  return (
    <div className="rounded-xl border border-black/10 bg-panel p-5">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto select-none">
        ...
        <text x={W - PAD} y={sy(grand) - 4} fill="#6b7280" fontSize="10" textAnchor="end">общее среднее</text>
        {groups.map((g, i) => (
          <g key={i}>
            ...
            <text x={colX(i)} y={BASE + 14} fill={colors[i]} fontSize="10" textAnchor="middle">группа {i + 1}</text>
          </g>
        ))}
      </svg>

      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm">
        <span className="text-gray-700">разброс МЕЖДУ группами (SS_b): {ssB.toFixed(0)}</span>
        <span className="text-gray-700">разброс ВНУТРИ (SS_w): {ssW.toFixed(0)}</span>
        <span className="text-[#2ab8eb]">F = {F.toFixed(2)}</span>
        <span className={sig ? 'text-green-600' : 'text-gray-600'}>{sig ? 'F > критич. → группы различаются' : 'F мал → различий не видно'}</span>
      </div>

      <div className="grid sm:grid-cols-2 gap-3 mt-4 text-sm">
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Разброс между средними групп</span><span className="text-cyanink">{sep}</span></div>
          <input type="range" min="0" max="16" step="1" value={sep} onChange={(e) => setSep(Number(e.target.value))} className="w-full accent-accent" />
        </label>
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Разброс внутри групп σ</span><span className="text-cyanink">{within}</span></div>
      ...
      <p className="text-xs text-gray-500 mt-2">F = (разброс между группами) / (разброс внутри). Раздвиньте средние — F растёт; добавьте шум внутри — F падает. Так ANOVA отвечает «есть ли вообще различие» сразу для нескольких групп, не плодя попарных сравнений.</p>
    </div>
  )
}
