...
export default function PairwiseIntervals() {
  ...
  return (
    <div className="rounded-xl border border-black/10 bg-panel p-5">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto select-none">
        ...
        <text x={sx(0)} y={PAD - 18} fill="#2a2f3a" fontSize="10" textAnchor="middle">разница = 0</text>
        {pairs.map((p, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={W - 6} y={y + 4} fill={c} fontSize="10" textAnchor="end">{sig ? 'значимо' : 'не знач.'}</text>
            </g>
          )
        })}
      </svg>
      ...
      <div className="grid sm:grid-cols-2 gap-3 mt-3 text-sm">
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Разброс между группами</span><span className="text-cyanink">{sep}</span></div>
          <input type="range" min="0" max="14" step="1" value={sep} onChange={(e) => setSep(Number(e.target.value))} className="w-full accent-accent" />
        </label>
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Разброс внутри групп σ</span><span className="text-cyanink">{sd}</span></div>
          <input type="range" min="3" max="14" step="1" value={sd} onChange={(e) => setSd(Number(e.target.value))} className="w-full accent-accent" />
        </label>
      </div>
      <p className="text-xs text-gray-500 mt-2">Каждая строка — разница средних пары групп с интервалом. Не накрыл ноль (зелёный) → значимо. Выключите поправку Тьюки — интервалы сузятся, и «значимых» пар станет больше: это и есть раздувание ложных находок при попарных тестах без поправки.</p>
    </div>
  )
}
