    ...
    <div className="rounded-xl border border-black/10 bg-panel p-5">
          ...
          <div className="grid gap-1.5" style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`, maxWidth: 320 }}>
            {!analysis
              ...
              : analysis.marked.map((t, i) => (
                <div key={i} className="aspect-square rounded" style={{ background: colorOf(t) }} title={`p = ${t.p.toFixed(3)}${t.isTrue ? ' · реальный эффект' : ''}`} />
              ))}
          </div>
          <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2 text-[11px]">
            <span className="text-[#16a34a]">■ верное открытие</span>
            <span className="text-[#f87171]">■ ложное открытие</span>
            <span className="text-[#d9920a]">■ пропущенный эффект</span>
            <span className="text-gray-500">■ верно отклонено</span>
          </div>
        </div>
      ...
      {analysis && (
        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-sm">
          <span className="text-[#16a34a]">верных открытий: {analysis.TP} из {Math.min(nTrue, m)}</span>
          <span className="text-[#f87171]">ложных: {analysis.FP}</span>
          <span className="text-[#d9920a]">пропущено: {analysis.FN}</span>
          <span className="text-gray-600">доля ложных среди найденного (FDR): {analysis.rejected ? (analysis.fdr * 100).toFixed(0) + '%' : '—'}</span>
        </div>
      )}

      <div className="grid sm:grid-cols-2 gap-x-5 gap-y-3 mt-4 text-sm">
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Число гипотез m</span><span className="text-cyanink">{m}</span></div>
        ...
        <label>
          <div className="flex justify-between text-gray-700 mb-1"><span>Из них реально работают</span><span className="text-cyanink">{Math.min(nTrue, m)}</span></div>
