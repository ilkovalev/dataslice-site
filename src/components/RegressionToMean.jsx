...
export default function RegressionToMean() {
  ...
  return (
    <div className="rounded-xl border border-black/10 bg-panel p-5">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-sm mx-auto h-auto select-none">
        ...
        <text x={sx(2.4)} y={sy(2.4) - 6} fill="#9ca3af" fontSize="9" textAnchor="middle">тест 2 = тест 1</text>
        {/* среднее по тесту 2 */}
        <line x1={PAD} y1={sy(0)} x2={W - PAD} y2={sy(0)} stroke="#9ca3af" strokeWidth="1" strokeDasharray="2 3" />
        <text x={W - PAD} y={sy(0) - 3} fill="#9ca3af" fontSize="8" textAnchor="end">среднее</text>
        ...
        <text x={sx(topT1) + 8} y={sy((topT1 + topT2) / 2)} fill="#dc4d4d" fontSize="9">откат к среднему</text>
        <text x={W - PAD} y={H - PAD + 16} fill="#9a907c" fontSize="10" textAnchor="end">результат теста 1 (первый раз) →</text>
        <text x={PAD + 2} y={PAD - 8} fill="#9a907c" fontSize="10" textAnchor="start">результат теста 2 (повтор) ↑</text>
        <text x={sx(thr) + 4} y={PAD + 10} fill="#d9a300" fontSize="9">топ-20% по тесту 1</text>
      </svg>
      <div className="text-sm text-gray-700 mt-1">
        Лучшие по тесту 1 (жёлтая зона): в среднем <span className="text-[#2ab8eb]">{topT1.toFixed(2)}</span> на тесте 1, но уже <span className="text-[#dc4d4d]">{topT2.toFixed(2)}</span> на повторе — откатились к среднему (0) сами, хотя с ними ничего не делали.
      </div>
      <label className="block mt-3 text-sm">
        <div className="flex justify-between text-gray-700 mb-1"><span>Доля удачи в результате</span><span className="tabular-nums text-cyanink">{(luck * 100).toFixed(0)}%</span></div>
