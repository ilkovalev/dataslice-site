...
// (все наблюдения по местам). Данные асимметричны вправо — виден разрыв median/mean.
const W = 520
const H = 250
const PAD = 40
const BINS = 26
...
const L = {
  ru: { slider: 'Перцентиль p', reset: 'сбросить', mean: 'среднее', median: 'медиана', rank: 'ранг',
    ...
    note: (p, val, u) => `p${p} = ${val}${u}: ниже этого значения лежит ${p}% данных (закрашено). На асимметричных данных хвостовые перцентили (p90, p99) важнее среднего — это «насколько плохо приходится худшим». Так задают SLA: «95% запросов быстрее X».`,
  },
